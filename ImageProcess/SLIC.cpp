...
CDib* SLIC::showLabels(int* finalLabels)
{
	CDib* pTest = new CDib();
	pTest->createGradeBmp(m_width, m_height, false);
	for (int w = 0; w < m_width; ++w)
	{
		for (int h = 0; h < m_height; ++h)
		{
			*(pTest->GetData() + h * m_width + w) = (BYTE)(finalLabels[h * m_height + w] * 255 / m_spcount);
		}
	}
	return pTest;
}
void SLIC::superPixels(CDib* lpDib, CDib** m_pAllImages, int superpixels)
{
	*pProcessImg = *lpDib;
	int height = pProcessImg->GetHeight();
	int width = pProcessImg->GetWidth();
	UINT* img = nullptr;
	int sz = width * height;
	img = new UINT[sz];
	memset(img, 0, sz * sizeof(UINT));
	LPBYTE lpData = pProcessImg->GetData();
	UINT* lpImg = img;
	for (int i = 0; i < sz; ++i)
	{
		memcpy((LPBYTE)lpImg, lpData, 3);
		lpData += 3;
		lpImg += 1;
	}
	m_spcount = superpixels;
	...
	int numlabels(0);
	DoSuperpixelSegmentation_ForGivenNumberOfSuperpixels(img, width, height, labels,
		numlabels, m_spcount, m_compactness);
	DrawContoursAroundSegments(img, labels, width, height, 0);
	if (labels) delete[] labels;
	m_pAllImages[1] = pProcessImg;
	//m_pAllImages[2] = pTest;
	//pTest->m_title = "label";
	lpData = pProcessImg->GetData();
	lpImg = img;
	for (int i = 0; i < sz; ++i)
	{
		memcpy(lpData, (LPBYTE)lpImg, 3);
		lpData += 3;
		lpImg += 1;
	}
	pProcessImg->m_title = "SLIC";
}
