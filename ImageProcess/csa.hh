...
#endif
        result_a[i] = (int)(v - head_lhs_node + 1);
        result_b[i] = (int)(v->matched->head - head_rhs_node + 1 +
...
if (v->matched)
  ...
  if (w->matched == v)
    ...
    {
    lhs_id = (int)(w->matched - head_lhs_node + 1);
...
if ((v+1)->priced_out - v->first > 0)
  ...
  for (a = v->first; a != (v+1)->priced_out; a++)
    {
    rhs_id = (int)(a->head - head_rhs_node + 1);
...
if (v->matched)
  {
  lhs_id = (int)(v->matched - head_lhs_node + 1);
...
*/
    ...
    return (unsigned)(t.QuadPart / 1e7);
}
