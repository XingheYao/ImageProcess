...
void seg2bdry(Mat &seg, Mat &bdry, CStr fmt)
{
	...
	bdry.create(2 * tx + 1, 2 * ty + 1, CV_32S);
	bdry = Scalar::all(0);

	Mat edgels_v(tx, ty, CV_32S, Scalar(0)), edgels_h(tx, ty, CV_32S, Scalar(0));
	for (int i = 0; i < tx - 1; i++) {
		int *ptr_res = edgels_v.ptr<int>(i);
		int *ptr_curr = seg.ptr<int>(i);
		int *ptr_next = seg.ptr<int>(i + 1);
		for (int j = 0; j < ty; j++)
			ptr_res[j] = (ptr_curr[j] != ptr_next[j]) ? 1 : 0;
			
	}
	for (int i = 0; i < tx; i++) {
		int *ptr_res = edgels_h.ptr<int>(i);
		int *ptr_curr = seg.ptr<int>(i);
		int *ptr_next = seg.ptr<int>(i);
		for (int j = 0; j < ty - 1; j++)
			ptr_res[j] = (ptr_curr[j] != ptr_next[j + 1]) ? 1 : 0;
	}

	for (int i1 = 2, i2 = 0; i2 < tx; i1 += 2, i2++) {
		int *ptr_bdry = bdry.ptr<int>(i1);
		int *ptr_edgv = edgels_v.ptr<int>(i2);
		for (int j1 = 1, j2 = 0; j2 < ty; j1 += 2, j2++)
			ptr_bdry[j1] = ptr_edgv[j2];
	}
	for (int i1 = 1, i2 = 0; i2 < tx; i1 += 2, i2++) {
		int *ptr_bdry = bdry.ptr<int>(i1);
		int *ptr_edgh = edgels_h.ptr<int>(i2);
		for (int j1 = 2, j2 = 0; j2 < ty; j1 += 2, j2++)
			ptr_bdry[j1] = ptr_edgh[j2];
	}
	for (int i1 = 2, i2 = 0; i2 < tx - 1; i1 += 2, i2++) {
		int *ptr_bdry = bdry.ptr<int>(i1);
		int *ptr_edgh_curr = edgels_h.ptr<int>(i2);
		int *ptr_edgh_next = edgels_h.ptr<int>(i2 + 1);
		int *ptr_edgv = edgels_v.ptr<int>(i2);
		for (int j1 = 2, j2 = 0; j2 < ty - 1; j1 += 2, j2++) {
			int m1 = max(ptr_edgh_curr[j2], ptr_edgh_next[j2]);
			int m2 = max(ptr_edgv[j2], ptr_edgv[j2 + 1]);
	...
	for (int i = 0; i < dy; i++)
		bdry.at<int>(0, i) = bdry.at<int>(1, i);
	for (int i = 0; i < dx; i++)
		bdry.at<int>(i, 0) = bdry.at<int>(i, 1);
	for (int i = 0; i < dy; i++)
		bdry.at<int>(dx - 1, i) = bdry.at<int>(dx - 2, i);
	for (int i = 0; i < dx; i++)
		bdry.at<int>(i, dy - 1) = bdry.at<int>(i, dy - 2);

	if (fmt == "imageSize") {
		Mat tmp(tx, ty, CV_32S);
		for (int i1 = 2, i2 = 0; i2 < tx; i1 += 2, i2++) {
			int *ptr_bdry = bdry.ptr<int>(i1);
			int *ptr_tmp = tmp.ptr<int>(i2);
			for (int j1 = 2, j2 = 0; j2 < ty; j1 += 2, j2++)
	...
	vecF &sumR, vecF &cntP, vecF &sumP, int nthresh, float maxDist, bool thinpb)
{
	...
	for (int t = 0; t < nthresh; t++) {
		...
		if (thinpb)
			thinning(bmap);
		Mat accp(bmap.size(), CV_32S, Scalar(0));
		...
		for (int i = 0; i < groundTruth.size(); i++) {
			...
			for (int r = 0; r < height; r++) {
				...
				int *ptr_gt = groundTruth[i].ptr<int>(r);
				int *ptr_ap = accp.ptr<int>(r);
...
void collect_eval_bdry(CStr pbDir)
{
	if (GetFileAttributesA(_S(pbDir + "eval_bdry.txt")) != INVALID_FILE_ATTRIBUTES)
		return;
	//ofstream out;
	//out.open("our_itr_sperate.txt", ios::app);
	vecS S;
	const int s_num = CmFile::GetNames(_S(pbDir + "*_ev1.bin"), S);
	int nthresh;
	FILE *file = fopen(_S(pbDir + S[0]), "rb");
	fread(&nthresh, sizeof(int), 1, file);
	fclose(file);
	vecI thresh(nthresh);
	...
	for (int i = 0; i < s_num; i++) {
		file = fopen(_S(pbDir + S[i]), "rb");
		fread(&nthresh, sizeof(int), 1, file);
		fread(thresh.data(), sizeof(int), nthresh, file);
		fread(cntR.data(), sizeof(float), nthresh, file);
		fread(sumR.data(), sizeof(float), nthresh, file);
		fread(cntP.data(), sizeof(float), nthresh, file);
		fread(sumP.data(), sizeof(float), nthresh, file);
		fclose(file);
