...
void thinningIteration(Mat &im, int iter)
{
	for (int i = 1; i < im.rows - 1; i++) {
		for (int j = 1; j < im.cols - 1; j++) {
			...
			int xh = 0, n, n1 = 0, n2 = 0, c3 = 0, x[8];
			x[0] = im.at<int>(i, j + 1);
			x[1] = im.at<int>(i - 1, j + 1);
			x[2] = im.at<int>(i - 1, j);
			x[3] = im.at<int>(i - 1, j - 1);
			x[4] = im.at<int>(i, j - 1);
			x[5] = im.at<int>(i + 1, j - 1);
			x[6] = im.at<int>(i + 1, j);
			x[7] = im.at<int>(i + 1, j + 1);
...
{
	CV_Assert_(im.type() == CV_32S, "The type of image must be Int...");
	Mat prev, diff;
	im.copyTo(prev);
