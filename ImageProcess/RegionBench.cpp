...
float variation_of_information(Mat n)
{
	...
	int N = (int)sum(n)[0];
	Mat joint(n.size(), CV_32F);
	for (int i = 0; i < _h; i++) {
		int *ptr_n = n.ptr<int>(i);
		float *ptr_joint = joint.ptr<float>(i);
		for (int j = 0; j < _w; j++)
			ptr_joint[j] = (float)ptr_n[j] / N;
	}
	vecF marginal_2(_w, 0), marginal_1(_h, 0);
	for (int i = 0; i < _h; i++) {
		float *ptr_joint = joint.ptr<float>(i);
		for (int j = 0; j < _w; j++) {
			marginal_2[j] += ptr_joint[j];
			marginal_1[i] += ptr_joint[j];
		}
	}

	float H1 = 0, H2 = 0, MI = 0, vi;
	for (int i = 0; i < _h; i++) {
		if (abs(marginal_1[i]) <= DOUBLE_EPS)
			H1 -= marginal_1[i] * log2(marginal_1[i] + 1);
		else
			H1 -= marginal_1[i] * log2(marginal_1[i]);
	}
	for (int i = 0; i < _w; i++) {
		if (abs(marginal_2[i]) <= DOUBLE_EPS)
			H2 -= marginal_2[i] * log2(marginal_2[i] + 1);
		else
			H2 -= marginal_2[i] * log2(marginal_2[i]);
	}

	Mat mul12(_h, _w, CV_32F);
	for (int i = 0; i < _h; i++) {
...
void match_segmentations2(Mat &seg, vecM &groundTruth, float *sumRI, float *sumVOI)
{
	...
	*sumRI = *sumVOI = 0;
	...
	for (int s = 0; s < _n; s++) {
		...
		int num1 = -1, num2 = -1;
		for (int i = 0; i < _h; i++) {
			int *ptr_seg = seg.ptr<int>(i);
			int *ptr_gt = gt.ptr<int>(i);
			for (int j = 0; j < _w; j++) {
				if (ptr_seg[j] > num1)
					num1 = ptr_seg[j];
				if (ptr_gt[j] > num2)
	...
	vecI &thresh, vecF &cntR, vecF &sumR, vecF &cntP, vecF &sumP, float *cntR_best, int nthresh)
{
	...
	for (int s = 0; s < nsegs; s++) {
		for (int i = 0; i < _h; i++) {
			int *ptr_gt = groundTruth[s].ptr<int>(i);
			for (int j = 0; j < _w; j++)
			    if (ptr_gt[j] > max_gts[s])
				   max_gts[s] = ptr_gt[j];
		}
...
void collect_eval_reg(CStr ucmDir)
{
	...
	vecS S;
	const int s_num = CmFile::GetNames(_S(ucmDir + "*_ev2.bin"), S);

	FILE *file = fopen(_S(ucmDir + S[0]), "rb");
	int nthresh;
	...
	for (int i = 0; i < s_num; i++) {
		...
		file = fopen(_S(ucmDir + S[i].substr(0, S[i].length() - 5) + "3.bin"), "rb");
		float tmp;
		fread(&tmp, sizeof(float), 1, file);
		std::fclose(file);
		cntR_best_total += tmp;

		file = fopen(_S(ucmDir + S[i].substr(0, S[i].length() - 5) + "4.bin"), "rb");
		fread(&nthresh, sizeof(int), 1, file);
		fread(thresh.data(), sizeof(int), nthresh, file);
		vecF tmpRI(nthresh), tmpVOI(nthresh);
		fread(tmpRI.data(), sizeof(float), nthresh, file);
		fread(tmpVOI.data(), sizeof(float), nthresh, file);
		std::fclose(file);
		for (int t = 0; t < nthresh; t++) {
			globalRI[t] += tmpRI[t];
